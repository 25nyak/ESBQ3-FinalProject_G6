...
contract LendingPlatform is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 g6tReward_deposit;      // G6T rewards from deposit
    ...
    function borrowUSDC(uint256 amount) external {
        ...
        if (user(msg.sender).interestFee != 0) {
          elapsedTime = block.timestamp - user(msg.sender).time_withdrawB_USDC;
          user(msg.sender).interestFee += elapsedTime * (user(msg.sender).withdrawB_USDC / 1000); // Pays 0.001% per second 86.4% per day
        } 
    ...
    function repayUSDC(uint256 amount) external {
        uint256 elapsedTime = block.timestamp - user(msg.sender).time_withdrawB_USDC;
        user(msg.sender).interestFee += elapsedTime * (user(msg.sender).withdrawB_USDC / 1000); // Pays 0.001% per second 86.4% per day
        uint256 totalDebt = user(msg.sender).borrowB_USDC + user(msg.sender).interestFee;
        ...
        if (amount >= totalDebt) {
          ...
          user(msg.sender).availableB_USDC = user(msg.sender).borrowB_USDC;
          lendingPool += user(msg.sender).borrowB_USDC;
